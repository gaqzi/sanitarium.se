...
class TestAuthorDisplay:
    ...
    def test_individual_post_shows_author_avatar(self, hugo_site, get_post_html_fixture):
        ...
        assert author_avatar.get('src') == expected_src, \
            f"Avatar src should be '{expected_src}' but was '{author_avatar.get('src')}'"
        assert author_avatar.get('alt') == "Björn Andersson", \
    ...
    def test_individual_post_shows_author_name(self, hugo_site, get_post_html_fixture):
        ...
        assert text_author_link is not None, "Should find author name link with 'Björn Andersson'"
        assert text_author_link.get('href') == "/about", \
    ...
    def test_all_individual_posts_show_authors(self, hugo_site, parse_html_fixture, get_all_posts_fixture):
    ...
    def test_author_link_consistency(self, hugo_site, get_post_html_fixture):
        ...
        assert urls[0] == "/about", f"Author links should point to '/about' but pointed to '{urls[0]}'"
    ...
    def test_post_without_author_still_renders(self, hugo_site, parse_html_fixture, get_all_posts_fixture):
        ...
        for post_path in posts:
            ...
            meta_content = meta_div.find('div', class_='meta-content')
            assert meta_content is not None, f"Post {post_path} should have meta-content"
            
            # Should always have at least one meta-line (for date/time)
            meta_lines = meta_content.find_all('div', class_='meta-line')
            assert len(meta_lines) >= 1, f"Post {post_path} should have at least one meta-line"
    
    def test_multiple_authors_support(self, hugo_site, parse_html_fixture, get_all_posts_fixture):
        ...
        for post_path in posts:
            ...
            if meta_div:
                ...
                assert len(author_links) >= 1, f"Post {post_path} should have at least one author"
                ...
                for link in author_links:
                    ...
                    assert has_text or has_img, "Author links should have either text or image content"
    
    def test_author_avatar_alt_text_accessibility(self, hugo_site, get_post_html_fixture):
    ...
    def test_individual_post_author_metadata_consistency(self, hugo_site, get_post_html_fixture):
        ...
        assert meta_author.get('content') == "Björn Andersson", \
        ...
        if twitter_author:
            assert twitter_author.get('content') == "Björn Andersson", \
        ...
        assert visible_author == "Björn Andersson", \

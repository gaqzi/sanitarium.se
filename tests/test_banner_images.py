...
class TestBannerImages:
    ...
    def test_default_banner_path_resolution(self, hugo_site, get_post_html_fixture):
        ...
        assert (
            "/img/banners/2025-07-which-hats-are-you-wearing.png" in twitter_image_url
    ...
    def test_banner_images_exist_in_static_files(self, hugo_site):
        ...
        banner_path = (
            ...
            / "img"
            / "banners"
            / "2025-07-which-hats-are-you-wearing.png"
        )

        assert banner_path.exists(), f"Banner image should exist at {banner_path}"

        # Verify it's a reasonable file size (not empty)
        assert (
            banner_path.stat().st_size > 1000
        ), f"Banner image should be substantial in size, got {banner_path.stat().st_size} bytes"

    def test_multiple_posts_have_banner_images(
        ...
        for post_path in posts[:10]:
            ...
            if og_image:
                ...
                if "/img/banners/" in image_url:
                    ...
                    banner_path = (
                        project_root / "static" / "img" / "banners" / banner_filename
                    )
    ...
    def test_twitter_card_type_with_banner_images(
        ...
        assert (
            twitter_image is not None
        ), "Should have twitter:image if using summary_large_image"
    ...
    def test_banner_image_content_type_and_accessibility(self, hugo_site):
        ...
        assert (
            file_size > 5000
        ), f"Banner should be substantial size, got {file_size} bytes"
        assert (
            file_size < 2_000_000
        ), f"Banner should not be too large, got {file_size} bytes"
        ...
        for banner_file in banner_files:
            ...
            assert filename.endswith(".png"), f"Banner should be PNG file: {filename}"

...
class TestMetaTags:
    ...
    def test_homepage_basic_meta_tags(self, hugo_site, parse_html_fixture):
        ...
        assert "width=device-width" in viewport.get(
            "content", ""
        ), "Viewport should include width=device-width"
        assert "initial-scale=1" in viewport.get(
            "content", ""
    ...
    def test_individual_post_basic_meta_tags(self, hugo_site, get_post_html_fixture):
        ...
        assert canonical_href.startswith("http"), "Canonical URL should be absolute"
        assert (
            "which-hat-are-you-wearing" in canonical_href
    ...
    def test_homepage_open_graph_tags(self, hugo_site, parse_html_fixture):
        ...
        if og_image is not None:
            ...
            assert (
                len(image_content.strip()) > 0
            ), "og:image should not be empty if present"
    ...
    def test_individual_post_open_graph_tags(self, hugo_site, get_post_html_fixture):
        ...
        assert url_content.startswith("http"), "og:url should be absolute URL"
        assert (
            "which-hat-are-you-wearing" in url_content
    ...
    def test_homepage_twitter_card_tags(self, hugo_site, parse_html_fixture):
        ...
        if twitter_title is not None:
            ...
            assert (
                len(title_content.strip()) > 0
            ), "twitter:title should not be empty if present"

        # twitter:description
        twitter_description = homepage.find(
            "meta", attrs={"name": "twitter:description"}
        )
        if twitter_description is not None:
            desc_content = twitter_description.get("content", "")
            assert (
                len(desc_content.strip()) > 0
            ), "twitter:description should not be empty if present"
        ...
        if twitter_image is not None:
            ...
            assert (
                len(image_content.strip()) > 0
            ), "twitter:image should not be empty if present"
    ...
    def test_individual_post_twitter_card_tags(self, hugo_site, get_post_html_fixture):
        ...
        if twitter_title is not None:
            ...
            assert (
                "hat" in title_content.lower()
            ), "twitter:title should relate to post content"
        ...
        if twitter_image is not None:
            ...
            assert image_content.startswith(
                "http"
        ...
        if twitter_creator is not None:
            ...
            assert (
                len(creator_content.strip()) > 0
            ), "twitter:creator should not be empty if present"
            assert creator_content.startswith(
                "@"
            ), "twitter:creator should start with @"
    ...
    def test_meta_tag_consistency_between_formats(
        ...
        assert (
            len(non_empty_descriptions) > 0
        ), "Should have at least one non-empty description"

        # All non-empty descriptions should be reasonable length (not just single words)
        for desc in non_empty_descriptions:
            assert (
                len(desc) > 10
            ), f"Description should be substantial, got: {desc[:50]}..."
            ...
            for script in json_ld_scripts:
                if script.string:
                    try:
                        ...
                        pytest.fail(
                            f"Invalid JSON-LD structure: {script.string[:100]}..."
                        )

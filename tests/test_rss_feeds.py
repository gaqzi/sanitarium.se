...
class TestRSSFeeds:
    ...
    def test_main_feed_exists_and_linked(self, hugo_site, parse_html_fixture):
        ...
        main_feed_link = homepage.find('link', {
            'rel': 'alternate',
        ...
        assert 'All Feed' in title or 'Feed' in title, f"Feed title should be descriptive, got: {title}"
    ...
    def test_blog_section_feed_exists_and_linked(self, hugo_site, parse_html_fixture):
        ...
        blog_feed_link = homepage.find('link', {
            'rel': 'alternate',
            'type': 'application/rss+xml'
        ...
        assert 'Blog Feed' in title, f"Blog feed title should mention 'Blog', got: {title}"
        
        # Check actual feed file exists
        blog_feed_file = hugo_site / "blog" / "feed.xml"
        assert blog_feed_file.exists(), "Blog RSS feed file should exist at /blog/feed.xml"
        ...
        if blog_index.exists():
            ...
            blog_feed_link_on_blog = blog_page.find('link', {
                'rel': 'alternate',
                'type': 'application/rss+xml'
            }, href=lambda x: x and 'blog/feed.xml' in x)
            assert blog_feed_link_on_blog is not None, "Blog section page should have blog RSS feed link"
    ...
    def test_tag_specific_feeds_only_on_tag_pages(self, hugo_site, parse_html_fixture):
        ...
        all_feed_links = homepage.find_all('link', {
            'rel': 'alternate',
            'type': 'application/rss+xml'
        })
        
        tag_feed_links = [
            link for link in all_feed_links
            if link.get('href', '').startswith('/tags/') and link.get('href', '').endswith('/feed.xml')
        ]
        
        assert len(tag_feed_links) == 0, f"Homepage should not have tag-specific feed links, found: {[link.get('href') for link in tag_feed_links]}"
        ...
        if thinking_tag_page.exists():
            ...
            tag_feed_link = tag_page.find('link', {
                'rel': 'alternate',
                'type': 'application/rss+xml',
                'href': '/tags/thinking-out-loud/feed.xml'
            })
            assert tag_feed_link is not None, "Tag page should have its own RSS feed link"
            
            # Check title mentions the tag
            title = tag_feed_link.get('title', '')
            assert 'Thinking-Out-Loud' in title or 'thinking-out-loud' in title.lower(), \
    ...
    def test_feed_content_has_items(self, hugo_site):
        ...
        assert first_item.find('description') is not None, "RSS item should have description"
        ...
        if pub_date and pub_date.string:
            ...
            assert len(pub_date.string.strip()) > 10, "pubDate should be properly formatted"
    ...
    def test_feed_links_are_absolute_urls(self, hugo_site, parse_html_fixture):
        ...
        feed_links = homepage.find_all('link', {
            'rel': 'alternate',
            'type': 'application/rss+xml'
        })
    ...
    def test_all_tag_pages_have_consistent_feed_links(self, hugo_site, parse_html_fixture):
        ...
        for tag_dir in tags_dir.iterdir():
            if tag_dir.is_dir() and tag_dir.name != "page":
                ...
                if index_file.exists():
                    ...
                    # Should have main feed link
                    main_feed_link = tag_page.find('link', {
                        'rel': 'alternate',
                        'type': 'application/rss+xml',
                        'href': '/feed.xml'
                    })
                    assert main_feed_link is not None, f"Tag page {tag_name} should have main feed link"
                    
                    # Should have blog feed link
                    blog_feed_link = tag_page.find('link', {
                        'rel': 'alternate',
                        'type': 'application/rss+xml'
                    }, href=lambda x: x and 'blog/feed.xml' in x)
                    assert blog_feed_link is not None, f"Tag page {tag_name} should have blog feed link"
                    ...
                    assert tag_feed_link is not None, f"Tag page {tag_name} should have its own feed link"
                    
                    # Check that the feed file actually exists
                    tag_feed_file = tag_dir / "feed.xml"
                    assert tag_feed_file.exists(), f"Tag feed file should exist for {tag_name}"
    ...
    def test_feed_has_required_channel_elements(self, hugo_site):
        ...
        for element in required_elements:
            ...
            assert elem.string and len(elem.string.strip()) > 0, \
    ...
    def test_blog_section_page_feed_links(self, hugo_site, parse_html_fixture):
        ...
        main_feed_link = blog_page.find('link', {
            'rel': 'alternate',
            'type': 'application/rss+xml',
            'href': '/feed.xml'
        })
        ...
        blog_feed_link = blog_page.find('link', {
            'rel': 'alternate',
            'type': 'application/rss+xml'
        }, href=lambda x: x and 'blog/feed.xml' in x)
        assert blog_feed_link is not None, "Blog section should have blog-specific feed link"
        
        # Should NOT have tag-specific feeds
        all_feed_links = blog_page.find_all('link', {
            'rel': 'alternate',
            'type': 'application/rss+xml'
        })
        ...
        tag_feeds = [
            link for link in all_feed_links
            if link.get('href', '').startswith('/tags/') and '/feed.xml' in link.get('href', '')
        ]
        
        assert len(tag_feeds) == 0, "Blog section should not have tag-specific feed links"
